...
class LabTestDiscoveryCubit extends Cubit<LabTestDiscoveryState> {
  ...
  Future<void> getTests() async {
    emit(LabTestDiscoveryLoading(state.tests, state.packages));
    final result = await repo.getTests();
    result.fold((error) {
      emit(LabTestDiscoveryFailed(state.tests, state.packages, error.message));
    }, (data) {
      emit(LabTestDiscoverySuccess(data, state.packages));
    });
  }
  ...
  Future<void> getPackages() async {
    ...
    result.fold((error) {
      ...
      emit(LabTestDiscoverySuccess(state.tests, data));
    });
  }
}
